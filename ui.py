    ...
    def do_qa(self, args):
        ...
        if args.category_id:
            ...
            if category_name != ".":
                ...
                new_cat = a.d.session.query(Category).filter(Category.name.like(f'%{category_name}%')).one()
            ...
            if not args.description:
                ...
                app.qa(session=a.d.session, url=args.url, category_id=str(new_cat.category_id), date=args.date)
            else:
                app.qa(session=a.d.session, url=args.url, category_id=str(new_cat.category_id),
                   date=args.date, description=' '.join(args.description))
